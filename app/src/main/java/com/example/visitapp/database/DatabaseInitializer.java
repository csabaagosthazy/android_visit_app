package com.example.visitapp.database;
...
public class DatabaseInitializer {
    ...
    public static void populateDatabase(final com.example.visitapp.database.AppDatabase db) {
        Log.i(TAG, "Inserting demo data.");
        PopulateDbAsync task = new PopulateDbAsync(db);
        task.execute();
    }

    private static void addPerson(final com.example.visitapp.database.AppDatabase db, final String email, final String firstName,
    ...
    private static void addVisit(final com.example.visitapp.database.AppDatabase db, final String description, final Date visitDate, final Long visitor, final Long employee){
        VisitEntity visit = new VisitEntity(description, visitDate, visitor, employee);
        db.visitDao().insert(visit);
    }


    private static void populateWithTestData(com.example.visitapp.database.AppDatabase db) throws ParseException {
    ...
    private static class PopulateDbAsync extends AsyncTask<Void, Void, Void> {
        ...
        private final com.example.visitapp.database.AppDatabase database;

        PopulateDbAsync(com.example.visitapp.database.AppDatabase db) {
            database = db;
        }
